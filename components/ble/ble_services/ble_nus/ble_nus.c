...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
	...
	p_nus->data_handler=&ble_nus_string_receive;
    p_nus->is_notification_enabled = false;
...
void ble_nus_string_receive(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
	...
	ii = 0;
	for(i = 0; i < length; ++i){
		if(p_data[i] - '0' >= 0 && p_data[i] - '0' <= 9){
			sp_data[ii] = p_data[i] - '0';
			ii++;
		}
	}
	num = 0;
	for(i = 0; i < ii; ++i){
		num *= 10;
		num += sp_data[i];
		//NRF_LOG_PRINTF("%d, ",sp_data[i]);
	}
	update_num(num);
//	NRF_LOG_PRINTF("\n");
//	NRF_LOG_PRINTF(p_data);
//	NRF_LOG_PRINTF("    ");
//	NRF_LOG_PRINTF("%d\n",num);
}
